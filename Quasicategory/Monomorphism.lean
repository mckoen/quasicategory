...
instance boundaryInclusion_mono (n : ℕ) : Mono (boundary n).ι := instMonoι ∂Δ[n]

/-- horn inclusions are monomorphisms. -/
instance hornInclusion_mono (n : ℕ) (i : Fin (n + 1)) : Mono (horn n i).ι := instMonoι Λ[n, i]

-- inner horn inclusions are monomorphisms
instance innerHornInclusion_mono ⦃n : ℕ⦄ ⦃i : Fin (n + 3)⦄ (_h0 : 0 < i) (_hn : i < Fin.last (n+2)) :
    monomorphisms SSet (horn (n + 2) i).ι := hornInclusion_mono _ _
