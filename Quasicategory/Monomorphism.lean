...
noncomputable def transfiniteCompositionOfMono_boundaryInclusions {X Y : SSet.{u}} (i : X ⟶ Y) [Mono i] :
    (coproducts.{u} boundaryInclusions).pushouts.TransfiniteCompositionOfShape ℕ i where
...
lemma transfiniteCompositions_pushouts_coproducts_boundaryInclusions :
    transfiniteCompositions.{u} (coproducts.{u} boundaryInclusions).pushouts = monomorphisms SSet.{u} := by
  apply le_antisymm
  · rw [transfiniteCompositions_le_iff, pushouts_le_iff, coproducts_le_iff]
    exact boundaryInclusions_le_monomorphisms
  · intro _ _ i (_ : Mono i)
    apply transfiniteCompositionsOfShape_le_transfiniteCompositions _ (ULift ℕ)
    exact ⟨(transfiniteCompositionOfMono_boundaryInclusions i).ofOrderIso (orderIsoULift.{u} ℕ).symm⟩

/-- `0077` (b) monomorphisms are generated by boundary inclusions. -/
lemma monomorphisms_eq_saturation_boundaryInclusions :
    monomorphisms SSet.{u} = saturation.{u} boundaryInclusions := by
  apply le_antisymm
  · rw [← transfiniteCompositions_pushouts_coproducts_boundaryInclusions,
      transfiniteCompositions_le_iff, pushouts_le_iff, coproducts_le_iff]
    exact le_saturation boundaryInclusions
  · rw [← Saturated.le_iff]
    intro _ _ _ ⟨n⟩
...
lemma trivialFibration_eq_rlp_monomorphisms :
  ...
  apply le_antisymm
  · rw [← le_llp_iff_le_rlp, monomorphisms_eq_saturation_boundaryInclusions,
      ← Saturated.le_iff]
    exact le_llp_rlp boundaryInclusions
  · exact antitone_rlp boundaryInclusions_le_monomorphisms
