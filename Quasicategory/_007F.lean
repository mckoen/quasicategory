...
lemma innerAnodyne_eq_T : innerAnodyne.{u} = (WeaklySaturatedClassOf.{u} bdryPushoutClass) := by
  rw [innerAnodyne_eq]
  apply le_antisymm
  all_goals refine minimalWeaklySaturated _ _ ?_ (by infer_instance)
  · intro _ _ f hf
    cases hf with
    | @mk n i h0 hn =>
      apply WeaklySaturatedOf.retract (hornRetract i h0 hn) -- reduces to showing horn inclusion is a retract of a boundary pushout maps
      exact monomorphisms_le_S Λ[n + 2, i].ι (hornInclusion_mono _ _)
  · intro _ _ f ⟨m⟩
    rw [← innerAnodyne_eq]
