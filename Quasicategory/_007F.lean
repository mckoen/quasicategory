...
def X_0 : SimplicialSubset (Δ[m] ⊗ Δ[2]) :=
  imagePresheaf (pushoutProduct.desc (hornInclusion 2 1) (boundaryInclusion m))
...
lemma monoPushout_innerAnodyne {A B : SSet} (i : A ⟶ B) [hi : Mono i] :
    innerAnodyne.{0} (pushoutProduct.desc i (hornInclusion 2 1)) := by
...
lemma contains_innerAnodyne_iff_contains_pushout_maps
    ...
    (∀ m, S (pushoutProduct.desc (boundaryInclusion m) (hornInclusion 2 1))) ↔ (∀ {X Y : SSet} (p : X ⟶ Y) (_ : innerAnodyne p), S p) := by
