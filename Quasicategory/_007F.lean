...
def X_0 : SimplicialSubset (Δ[m] ⊗ Δ[2]) :=
  imagePresheaf (PushoutProduct.pushoutProduct (hornInclusion 2 1) (boundaryInclusion m))
...
lemma innerAnodyne_eq_T : innerAnodyne.{w} = (WeaklySaturatedClassOf.{w} bdryPushoutClass.{w}) := by
  rw [innerAnodyne_eq.{w}]
  ext X Y f
  refine ⟨?_, ?_⟩ -- other direction is more technical
  intro h
  refine minimalWeaklySaturated.{_, _, w} bdryPushoutClass.WeaklySaturatedClassOf InnerHornInclusions ?_ (by infer_instance) _ h
  intro A B g hg
  induction hg with
  | @mk n i h0 hn =>
  ...
  refine minimalWeaklySaturated.{_, _, w} InnerHornInclusions.WeaklySaturatedClassOf bdryPushoutClass ?_ (by infer_instance) f
  intro _ _ f hf
