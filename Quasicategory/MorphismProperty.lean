...
instance llp_pushout {T : MorphismProperty C} : IsStableUnderCobaseChange T.llp := by
  refine ⟨?_⟩
...
instance rlp_pullback {T : MorphismProperty C} : IsStableUnderBaseChange T.rlp := by
  refine ⟨?_⟩
...
instance of_is (T : MorphismProperty C) : WeaklySaturated.{w} (WeaklySaturatedClassOf.{w} T) where
  IsStableUnderCobaseChange := ⟨fun h hf ↦ WeaklySaturatedOf.pushout h hf⟩
