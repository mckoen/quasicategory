...
instance (T : MorphismProperty C) : WeaklySaturated (WeaklySaturatedClassOf T) where
  ...
  IsStableUnderTransfiniteComposition := {
    ...
    isStableUnderTransfiniteCompositionOfShape := fun β ↦ {
      condition := by
        intro F hF h c hc
        have n := @WeaklySaturatedOf.transfinite C _ T β _ _ _ F hF c hc
        dsimp [WeaklySaturatedClassOf]
        -- universe issues
        sorry } }
...
lemma minimalWeaklySaturated (S T : MorphismProperty C) (h : T ≤ S) : WeaklySaturated S → (WeaklySaturatedClassOf T) ≤ S := by
  intro hS X Y f hf
  induction hf with
  ...
  | transfinite _ F hF c hc _ _ =>
    have := hS.IsStableUnderTransfiniteComposition.isStableUnderTransfiniteCompositionOfShape -- universe issues
    sorry
