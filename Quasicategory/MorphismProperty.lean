...
noncomputable
instance iso_pushout (X Y : C) (p : X ⟶ Y) (hp : isomorphisms C p) : Limits.IsColimit (Limits.PushoutCocone.mk p p (Eq.refl (𝟙 X ≫ p))) where
  desc s := by
    let g := Exists.choose hp.out
    let (hg : p ≫ g = 𝟙 X ∧ g ≫ p = 𝟙 Y) := Exists.choose_spec hp.out
    refine g ≫ ?_
    have := Limits.PushoutCocone.inl s
    sorry
...
lemma WeaklySaturated_contains_iso (T : MorphismProperty C) [hT: WeaklySaturated T] (p : X ⟶ Y) :
  ...
  hT.StableUnderCobaseChange ⟨⟨rfl⟩, ⟨iso_pushout X Y p hp⟩⟩ (hT.IsStableUnderTransfiniteComposition.id_mem X)
