...
abbrev innerHornImage : (Δ[n] ⊗ Δ[2]).Subcomplex :=
  Λ[n + 1, ⟨i.snd.succ, by omega⟩].image (ιSimplex i)

lemma innerHornImage_eq_iSup : innerHornImage i =
    ⨆ (j : ({⟨i.snd.succ, by omega⟩}ᶜ : Set (Fin (n + 2)))), (face {j.1}ᶜ).image (ιSimplex i) := by
...
lemma face_one_image_not_le_unionProd :
    ¬ (face {1}ᶜ).image (ιSimplex ⊥) ≤ ∂Δ[n + 1].unionProd Λ[2, 1] := by
