...
open Sigma.Lex in
def filtrationPushout_intermediate' (i : Σₗ (b : Fin (n + 1)), Fin b.succ) :
    Sq
    ...
    · rw [σ.subcomplex_le_innerHornImage_iff' _ _ inf_le_left, le_inf_iff, not_and]
      ...
      · next ha =>
        ...
        · next h =>
          ...
          subst hj
          sorry
      --have := σ.faceImage_succ_not_le_filtration₁'
      /-
      rw [σ.subcomplex_le_innerHornImage_iff' (by rw [Fin.le_iff_val_le_val, ← Nat.lt_succ]; exact (succ i).2.2) _ inf_le_left]
